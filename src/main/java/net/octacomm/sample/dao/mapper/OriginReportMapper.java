...
@CacheNamespace
public interface OriginReportMapper extends CRUDMapper<Report, ReportParam, Integer> {
	...
	public String TABLE_NAME = " TB_REPORT_ORIGIN ";
	...
	//@Select("SELECT COUNT(*) FROM (SELECT * FROM TB_REPORT_ORIGIN WHERE isDel = 0) A , (SELECT * FROM TB_DEVICE WHERE isDel = 0) B " + 
	...
	@Select("SELECT " +
			...
			"            AND isDel = 0 " +
			"    GROUP BY deviceIdx , pileNo , location) a")
	int getCountByConstruction(@Param("constructionIdx") int constructionIdx);

	
	@Select("SELECT deviceIdx, COUNT(*) AS cnt FROM TB_REPORT_ORIGIN GROUP BY deviceIdx ORDER BY COUNT(*) DESC LIMIT 1")
	ReportMaxCount getMaxCount();
	
	
	@Select("SELECT COUNT(*) FROM (" + 
			"SELECT D.reportIdx, COUNT(*) FROM TB_CONSTRUCTION A, TB_DEVICE B , TB_REPORT_ORIGIN C , TB_PENETRATION_ORIGIN D " + 
	...
	int isBigAllReports(@Param("constructionIdx") int constructionIdx);

	
	@Select("SELECT COUNT(*) FROM TB_REPORT_ORIGIN WHERE deviceIdx = #{deviceIdx} AND location =  #{location} AND pileNo = #{pileNo} AND isDel = 0 AND IF((SELECT usePileStandard FROM TB_DUPLICATE_CHECK_CONFIG WHERE deviceIdx=#{deviceIdx} OR (deviceIdx=0 AND constructionIdx=(SELECT constructionIdx FROM TB_DEVICE WHERE id=#{deviceIdx})) OR (deviceIdx=0 AND constructionIdx=0) ORDER BY deviceIdx<>0 DESC,constructionIdx<>0 DESC LIMIT 1), pileStandard = #{pileStandard}, 1) = 1")
	...
	//@Update("UPDATE " + TABLE_NAME + " SET isDuple = 1 where id = #{id}")
	//void updateDupl(UpdateReport report2);
	
	@Update("UPDATE " + TABLE_NAME + " SET isDuple = 0 where id = #{id}")
	void updateDupl2(Report report2);
	
	//@Update("UPDATE " + TABLE_NAME + " SET isDuple = 0 where id = #{id}")
	//void updateDupl2(UpdateReport report2);

	@Select("select count(*) from TB_REPORT_ORIGIN WHERE TB_REPORT_ORIGIN.deviceIdx = TB.deviceIdx and TB_REPORT_ORIGIN.pileNo = TB.pileNo and TB_REPORT_ORIGIN.location = TB.location and TB_REPORT_ORIGIN.isDel = 0" )
	int getIsDuple(Report rp);
	
	//@Select("select count(*) from TB_REPORT_ORIGIN WHERE TB_REPORT_ORIGIN.deviceIdx = TB.deviceIdx and TB_REPORT_ORIGIN.pileNo = TB.pileNo and TB_REPORT_ORIGIN.location = TB.location and TB_REPORT_ORIGIN.isDel = 0" )
	//int getIsDuple(UpdateReport rp);
	
	@Select("SELECT * FROM TB_REPORT_ORIGIN WHERE deviceIdx = #{deviceIdx} AND location =  #{location} AND pileNo = #{pileNo} AND isDel = 0 AND id <> #{id} LIMIT 1")
	Report getDuplicationRepots(Report report);

	@Select("SELECT COUNT(*) FROM TB_REPORT_ORIGIN WHERE deviceIdx = #{deviceIdx} AND location =  #{location} AND pileNo = #{pileNo} AND isDel = 0  AND id <> #{id} ")
	int isDuplication2(Report rp);

	
	@Select("SELECT * FROM TB_REPORT_ORIGIN WHERE deviceIdx = #{deviceIdx} AND location =  #{location} AND pileNo = #{pileNo} AND isDel = 0 AND id <> #{id}")
	List<Report> getDuplicationRepotsAll(Report rp);
	
	
	@Select("SELECT * FROM TB_REPORT_ORIGIN WHERE deviceIdx = #{deviceIdx} AND location =  #{location} AND pileNo = #{pileNo} order by isDuple asc")
