...
public interface OriginReportMapper extends CRUDMapper<Report, ReportParam, Integer> {
	...
	public String UPDATE_VALUES = " pileType = #{pileType} ,"
			...
			+ "isDuple = (select * from (select IF(count(*) > 1, 1, 0) from TB_REPORT_ORIGIN where deviceIdx = #{deviceIdx} and pileNo = #{pileNo} and location = #{location} AND isDel = 0 )  as a ) , "
			+ "bigo = #{bigo} , "
			+ "sprCol1 = #{sprCol1} ";
	...
	@Select("SELECT COUNT(*) FROM TB_REPORT_ORIGIN WHERE deviceIdx = #{deviceIdx} AND location =  #{location} AND pileNo = #{pileNo} AND isDel = 0 ")
