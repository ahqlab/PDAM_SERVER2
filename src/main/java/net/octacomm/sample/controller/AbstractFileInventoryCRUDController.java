...
public abstract class AbstractFileInventoryCRUDController<M extends CRUDMapper<D, P, PK>, D extends Domain, P extends DomainParam, PK> {
	...
	public String list(Model model, @ModelAttribute("domainParam") P param, BindingResult result, HttpSession session) {
		...
		if((int) session.getAttribute("constructionIdx") == 1308) {
			return "fileinventory/listJeil";
		}
		return "fileinventory/list";
	}
	...
	protected abstract String getRedirectUrl(HttpServletRequest request, HttpSession session, @ModelAttribute("domain") D domain);
