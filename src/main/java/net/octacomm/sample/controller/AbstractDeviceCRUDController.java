...
import net.octacomm.sample.utils.DevicePagination;
...
public abstract class AbstractDeviceCRUDController<M extends CRUDMapper<D, P, PK>, D extends Domain, P extends DomainParam, PK> {
	...
	public String list(Model model, @ModelAttribute("domainParam") P param, BindingResult result, HttpSession session) {
		...
		int totalCount = 0;
		if(param.getConstructionIdx() == 588 || param.getConstructionIdx() == 613 || param.getConstructionIdx() == 627) {
			totalCount = mapper.getCountByParamForSamsung(param);
		}else {
			totalCount = mapper.getCountByParam(param);
		}
		...
		if (param.getPageSize() > 0 && param.getPageGroupSize() > 0) {
			page = new DevicePagination(param.getPageSize(), param.getPageGroupSize(), totalCount, param.getCurrentPage());
		} else {
			page = new DevicePagination(totalCount, param.getCurrentPage());
		}
		List<D> domainList;
		if(param.getConstructionIdx() == 588 || param.getConstructionIdx() == 613 || param.getConstructionIdx() == 627) {
			domainList = mapper.getListByParamForSamsung(page.getStartRow(), page.getPageSize(), param);
		}else {
			domainList = mapper.getListByParam(page.getStartRow(), page.getPageSize(), param);
		}
	
		logger.debug("Domain List Size : {}", domainList.size());
		
		TotalWorkQuantity totalWorkQuantity = totalWorkQuantityMapper.get(param.getConstructionIdx());
		int processRate = 0;
		
		try {
			int quantityLeft = reportMapper.getCountByConstruction(totalWorkQuantity.getConstructionIdx());
			totalWorkQuantity.setExecutedQuantity(quantityLeft);
			double x2 = quantityLeft;
			double y2 = totalWorkQuantity.getQuantity();
			if(y2 == 0) {
				processRate = 0;
			}else {
				processRate = (int) (x2 / y2 * 100.0); //공정률
			}
			totalWorkQuantity.setProcessRate(processRate);
			totalWorkQuantity.setQuantityLeft( (int) (y2 - x2));
			totalWorkQuantity.setNgQuantity(reportMapper.getNgQuantity(totalWorkQuantity.getConstructionIdx()));
			
			model.addAttribute("totalWorkQuantity", totalWorkQuantity);	
		}catch(Exception e) {
			model.addAttribute("totalWorkQuantity", new TotalWorkQuantity());	
		}
		ConOptionCondition conOptionCondition = constructionMapper.getConOptionCondition(param.getConstructionIdx());
		
		String tripodCount = spareDeviceMapper.getTripodCount(param.getConstructionIdx() , 0);
		String tripodChangeCount = spareDeviceMapper.getTripodCount(param.getConstructionIdx() , 1);
		model.addAttribute("tripodCount", (tripodCount != null ? tripodCount : "0"));	
		model.addAttribute("tripodChangeCount", (tripodChangeCount != null ? tripodChangeCount : "0"));	
		model.addAttribute("conOptionCondition", conOptionCondition);	
		model.addAttribute("page", page);	
		model.addAttribute("domainList", domainList);
		if((int) session.getAttribute("role") == 4) {
			return "device/listVim";
		}
		return "device/list";
	}
