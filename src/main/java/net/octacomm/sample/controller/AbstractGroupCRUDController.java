...
import net.octacomm.sample.utils.GroupPagination;
...
public abstract class AbstractGroupCRUDController<M extends CRUDMapper<D, P, PK>, D extends Domain, P extends DomainParam, PK> {
	...
	public void list(Model model, @ModelAttribute("domainParam") P param, BindingResult result, HttpSession session) {
		...
		if (param.getPageSize() > 0 && param.getPageGroupSize() > 0) {
			page = new GroupPagination(param.getPageSize(), param.getPageGroupSize(), totalCount, param.getCurrentPage());
		} else {
			page = new GroupPagination(totalCount, param.getCurrentPage());
		}
