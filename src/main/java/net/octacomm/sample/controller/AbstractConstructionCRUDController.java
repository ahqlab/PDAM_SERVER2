...
import net.octacomm.sample.utils.ConstructionPagination;
...
public abstract class AbstractConstructionCRUDController<M extends CRUDMapper<D, P, PK>, D extends Domain, P extends DomainParam, PK> {
	...
	public void list(Model model, @ModelAttribute("domainParam") P param, BindingResult result, HttpSession session) {
		...
		//가맹점(role : 3) 및 협약업체(role : 4) 은 FcIdx 셋팅한다.
		if((int) session.getAttribute("role") == 3 || (int) session.getAttribute("role") == 4) {
			param.setFcIdx((int) session.getAttribute("fcIdx"));
		}else {
			if((int) session.getAttribute("groupIdx") > 0) {
				param.setGroupIdx((int) session.getAttribute("groupIdx"));
			}
		}
		...
		ConstructionPagination page;
		
		if (param.getPageSize() > 0 && param.getPageGroupSize() > 0) {
			page = new ConstructionPagination(param.getPageSize(), param.getPageGroupSize(), totalCount, param.getCurrentPage());
		} else {
			page = new ConstructionPagination(totalCount, param.getCurrentPage());
		}
