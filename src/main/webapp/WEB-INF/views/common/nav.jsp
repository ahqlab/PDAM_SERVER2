...
<style>.left-menu .mlist img,.left-menu .mlist svg{width:20px;height:20px;object-fit:contain;margin-right:10px}</style>
<div class="left-menu">
		...
		<c:choose>
			<c:when test="${sessionInfo.role == 0}">
				...
				<c:if test="${not empty param.constructionIdx}">
					<div class="mlist"><a href="${pageContext.request.contextPath}/device/list?constructionIdx=${param.constructionIdx}" class="${fn:contains(currentPath, '/device/') ? 'menuActive' : ''}"><img src="${pageContext.request.contextPath}/images/menu_icon10.png" />기기관리</a></div>
			...
			<c:when test="${sessionInfo.role == 1}">
				<div class="mlist"><a href="${pageContext.request.contextPath}/device/list?constructionIdx=${sessionInfo.constructionIdx}" class="${fn:contains(currentPath, '/device/') ? 'menuActive' : ''}"><img src="${pageContext.request.contextPath}/images/menu_icon10.png" />기기관리</a></div>
				<c:if test="${not sessionScope.settingRequired or (sessionScope.isHiddenManager ? sessionScope.constructionSetting.useAdminFileMenu : sessionScope.constructionSetting.useGuestFileMenu)}"><div class="mlist"><a href="${pageContext.request.contextPath}/fileinventory/list?constructionIdx=${sessionInfo.constructionIdx}" class="${fn:contains(currentPath, '/fileinventory/') ? 'menuActive' : ''}"><img src="${pageContext.request.contextPath}/images/menu_icon08.png" />파일반입 및 수정</a></div></c:if>
			...
			<c:when test="${sessionInfo.role == 2}"><div class="mlist"><a href="${pageContext.request.contextPath}/construction/list?groupIdx=${sessionInfo.groupIdx}" class="${fn:contains(currentPath, '/construction/') ? 'menuActive' : ''}"><img src="${pageContext.request.contextPath}/images/menu_icon07.png" />협력사</a></div></c:when>
			<c:when test="${sessionInfo.role == 3}"><div class="mlist"><a href="${pageContext.request.contextPath}/construction/list?fcIdx=${sessionInfo.fcIdx}" class="${fn:contains(currentPath, '/construction/') ? 'menuActive' : ''}"><img src="${pageContext.request.contextPath}/images/menu_icon07.png" />협력사</a></div></c:when>
			<c:otherwise>
