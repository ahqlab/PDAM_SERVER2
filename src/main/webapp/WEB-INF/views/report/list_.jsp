...
function onClickReportUpdate(){
	...
	function downloadExcel() {
		...
		if(type != 'all'){
			...
			if (constructionIdx != null && constructionIdx != '') {
				searchStr += "&constructionIdx=" + constructionIdx;
			}
			var jb = $('#startDate').val();
			var endDate = $('#endDate').val();
			if (jb != '' && endDate != '') {
				searchStr += "&startDate=" + jb + "&endDate=" + endDate;
			}
			var location = $('#location').val();
			if (location != '') {
				searchStr += "&location=" + location;
			}
			var pileNo = $('#location').val();
			if (pileNo != '') {
				searchStr += "&pileNo=" + pileNo;
			}  */
			//var para = document.location.href.split("?");
			//location.href = '${pageContext.request.contextPath}/report/download/excel?' + searchStr;
			...
			<c:otherwise>
				...
				<table  class="table03" ${isBig == true ? '' : 'style="width: 3100px;'}">
					<tr>
						<c:choose>
							<c:when test="${sessionInfo.role == 0  || sessionInfo.hiddenManager == true  || sessionInfo.role == 3}">
								<th style="width: 50px;" rowspan="2">선택</th>
							</c:when>
						</c:choose>
