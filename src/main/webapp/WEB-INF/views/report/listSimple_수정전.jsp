	...
	<div class="TopContArea">
		<div class="titArea mb-40">
			...
			<div class="titBtnArea">
				<c:choose>
					<c:when test="${sessionInfo.role == 1 or sessionInfo.role == 4}">
						...
						<%-- <div class="printPdfBtn" onclick="javascript:downloadDrivingAllRecoredBook('${pageContext.request.contextPath}', '${sessionInfo.constructionIdx}', '${device.machineNumber}');" >전체 기록지 PDF</div> --%>
					</c:when>
					<c:otherwise>
						...
						<%-- <div class="printPdfBtn" onclick="javascript:downloadDrivingAllRecoredBook('${pageContext.request.contextPath}', '${param.constructionIdx}', '${device.machineNumber}');" >전체 기록지 PDF</div> --%>
					</c:otherwise>
				</c:choose>
			</div>
		</div>
								...
								<c:when test="${sessionInfo.role == 0}">
									<tr>
										<td style="width: calc(100% / 3);">${domain.currentDateTime}
											...
											<a href="javascript:downloadDrivingRecoredBook('${pageContext.request.contextPath}', '${param.constructionIdx}', '${device.machineNumber}', '${domain.currentDateTime}');" class="tableCheckGreen">일일 기록지 PDF</a>
										</td>
									...
									<tr>
										<c:choose>
											<c:when test="${sessionInfo.hiddenManager == true && originDataYn > 0}">
												<td style="width: calc(100% / 3);">
													<c:choose>
														<c:when test="${sessionInfo.hiddenManager == true && sessionInfo.showPdfYn == true}">
															...
															<a href="javascript:downloadDrivingRecoredBook('${pageContext.request.contextPath}', '${sessionInfo.constructionIdx}', '${device.machineNumber}', '${domain.currentDateTime}');" class="tableCheckGreen">일일 기록지 PDF</a>
														</c:when>
														<c:otherwise>
															${domain.currentDateTime}
															<a href="${pageContext.request.contextPath}/report/list?id=${param.id}&date=${domain.currentDateTime}&constructionIdx=${sessionInfo.constructionIdx}" class="tableCheck">기록지확인</a>
