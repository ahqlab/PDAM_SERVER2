...
<%--			<!-- 계약서 관리 -->
		...
		<c:when test="${sessionInfo.role == 1}">
			<div class="pc-menu">
				...
				<c:choose>
					<c:when test="${sessionScope.settingRequired}">
						<%-- 신규 현장: TB_CONSTRUCTION_SETTING 권한 기반 --%>
						<c:set var="showFileMenu" value="${sessionScope.isHiddenManager ? sessionScope.constructionSetting.useAdminFileMenu : sessionScope.constructionSetting.useGuestFileMenu}" />
						<c:if test="${showFileMenu}">
							<div class="mlist">
								<a href="${pageContext.request.contextPath}/fileinventory/list?constructionIdx=${sessionInfo.constructionIdx}">
									<img src="${pageContext.request.contextPath}/new/img/menuIcon06.png">파일반입 및 수정
								</a>
							</div>
						</c:if>
					</c:when>
					<c:otherwise>
						<%-- 구 현장: 기존 하드코딩 로직 유지 --%>
						<c:if test="${not ((sessionInfo.constructionIdx == 1003 and sessionInfo.hiddenManager == false) or (sessionInfo.constructionIdx == 988 and sessionInfo.hiddenManager == false))}">
							<div class="mlist">
								<a href="${pageContext.request.contextPath}/fileinventory/list?constructionIdx=${sessionInfo.constructionIdx}">
									<img src="${pageContext.request.contextPath}/new/img/menuIcon06.png">파일반입 및 수정
								</a>
		...
		<c:when test="${sessionInfo.role == 2}">
			<div class="pc-menu">
				...
				<div class="mlist">
					<a href="${pageContext.request.contextPath}/device/list?constructionIdx=${param.constructionIdx}" class="menuActive">
						<img src="${pageContext.request.contextPath}/new/img/menuIcon06.png"/>기기관리
					</a>
				</div>
				<div class="mlist">
