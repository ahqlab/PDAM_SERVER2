...
<script type="text/javascript">
	function formCheck(){
		...
		$("#constructionIdx option").not(":selected").attr("disabled", "");
		$("#constructionIdx").attr("disabled", "false"); //1.
		$("#constructionIdx").removeAttr("disabled");  //2.
		return true;
	}
					...
					if(data.length > 0){
						...
						if(role > 0){
							...
							$.each(data, function(index, item) {
								...
								var idx = '<c:out value="${constructionIdx}"/>';
								if(item.id == idx){
...
<%-- 	<div class="tab_menu">
	...
	<div class="table01_content">
		...
		<!--table_white-->
		<form:form method="POST"  action="${pageContext.request.contextPath}/device/regist" id="customerForm" commandName="domain" >
				...
				<tr>
					...
					<td>
						<c:choose>
							...
							<c:otherwise>
								<select id="constructionIdx" disabled="disabled" class="input01" style="color: #000000;" name="constructionIdx">
								</select>
							</c:otherwise>
						</c:choose>
					</td>
