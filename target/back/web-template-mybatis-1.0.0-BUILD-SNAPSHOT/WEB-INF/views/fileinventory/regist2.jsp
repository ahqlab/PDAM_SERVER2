...
<script>
	$(document).ready(function() {
		...
		$('#pileType').change(function () {
			...
			location.href = '${pageContext.request.contextPath}/fileinventory/regist2?constructionIdx=' + constructionIdx +'&pileType=' + pileType;
		});
	});	
	...
	function checkValidation(){
		...
		var pileStandard  = $('#pileStandard').val();
		...
		jQuery.ajax({
			...
			success : function(data) {
				if(data){
					...
					//이미 등록된 것
					if(fileWeight != ""){
						alert(registDate + "일 " + pileType + " " + pileStandard + " " + fileWeight +  " 의 반입량이 이미 입력되있습니다. ");	  
					}else{
						alert(registDate + "일 " + pileType + " " + pileStandard + " 의 반입량이 이미 입력되있습니다. ");	  
					}
				};
			},
...
<div class="right_content">
	<div class="table01_content">
		...
		<div class="table_list">
			<form:form  id="regForm" method="POST" action="${pageContext.request.contextPath}/fileinventory/regist" commandName="domain" >
			<table class="table01">
				<tr>
					...
					<th>
						종류 :
						<c:choose>
							<c:when test="${param.pileType eq 'PHC'}">
							...
							<c:otherwise>
								<form:select path="pileType"  class="input01" style="width: 80px; font-size: 20px;" >
									...
									<form:option value="STEEL" selected="true" >STEEL</form:option>
								</form:select>
							</c:otherwise>
						</c:choose>
					</th>
			...
			<div class="btn_box">
				<input type="submit" class="button02 button05" style="margin-right: 10px;" value="등록하기" onclick="return checkValidation();">
				&nbsp;
				<input class="button01 button02"  type="button" style="background-color: black;" onclick="location.href='${pageContext.request.contextPath}/fileinventory/list?constructionIdx=${param.constructionIdx}'" value="목록"/>
			</div>
			</form:form>
		</div>
	</div>
