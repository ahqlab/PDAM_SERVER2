...
$(document).ready(
	...
	jQuery.ajax({
		...
		success : function(data) {
			if(data.length > 0){
				$('#fcIdx').append("<option value=\"0\" ${domain.fcIdx == "0" ? 'selected' : ''}>없음</option>");
				$.each(data, function(index, item) {
					var fcIdx = '${domain.fcIdx}';
					if(fcIdx == item.idx){
						$('#fcIdx').append("<option selected value='" + item.idx + "'>"+ item.fcName + "</option>");
					}else{
						$('#fcIdx').append("<option value='" + item.idx + "'>"+ item.fcName + "</option>");
					}
				});
			}
		},
		...
		error : function(xhr, status, error) {
			$('#fcIdx').append("<option value=\"0\">선택</option>");
			alert("에러발생");
		}
	});
		...
		<div class="table_white_form">
			<table class="table_white">
				<c:choose>
					<c:when test="${sessionInfo.role == 0}">
						<tr id="franchiseTr">
							...
							<td>
								<select id="fcIdx" class="input01" style="color: #000000;" name="fcIdx">
								</select>
							</td>
						</tr>
					</c:when>
				...
				<tr>
					<td>시간출력여부</td>
					<td>
						<form:select path="longCalYn" class="input01" >
