...
function numFormat(variable) {
 ...
 function goRegistPage(){
	...
	var year =  $("#dinYear option:selected").val();
 	...
 	var day =  $("#dinDay option:selected").val();
 	var operDate = numFormat(year) + "" + numFormat(month) + "" + numFormat(day);
	 document.location.href =  root + "/oiluse/regist?deviceIdx=" + deviceIdx + "&constructionIdx=" + constructionIdx + "&erpDiv=${param.erpDiv}&operDate=" + operDate; 
 }
...
<div class="right_content">
	<div class="tab_menu">
		<ul>
			...
			<li>
				<a class="on" href="${pageContext.request.contextPath}/oiluse/list?deviceIdx=${param.deviceIdx}&constructionIdx=${param.constructionIdx}&erpDiv=2">
					유류사용현황
				</a>
			</li>
			<li>
		...
		<div class="table_list">
			<table class="table01">
				...
				<c:forEach var="domain" items="${domainList}" varStatus="status">
					<tr onclick="javascript:goUpdatePage('${pageContext.request.contextPath}/oiluse/update?id=${domain.id}&deviceIdx=${domain.deviceIdx}&constructionIdx=${domain.constructionIdx}&erpDiv=${param.erpDiv}');">
						<td>${domain.dmiCol}</td>
						<td>${domain.preDay}</td>
						<td>${domain.today}</td>
